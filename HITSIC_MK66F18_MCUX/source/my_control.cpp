...
void CTRL_MENUSETUP(menu_list_t* List)
{
    static menu_list_t *TestList = MENU_ListConstruct("para_control", 20, List);
     ...
     static menu_list_t *pidList = MENU_ListConstruct("pidList", 20,List);
                 ...
                 {
                     MENU_ListInsert(pidList, MENU_ItemConstruct(varfType,&dirPID.kp, "dir.kp",14 ,menuItem_data_global));
                     MENU_ListInsert(pidList, MENU_ItemConstruct(varfType,&dirPID.kd, "dir.kd",15 ,menuItem_data_global));
                     MENU_ListInsert(pidList, MENU_ItemConstruct(varfType,&dirPID.ki, "dir.ki",22 ,menuItem_data_global));
                 }
...
/****************************************速度环**********************************************/
float speedL[3]={0.0f,-100.0f,100.f},speedR[3]={0.0f,-100.0f,100.f};
pidCtrl_t spdPID =
