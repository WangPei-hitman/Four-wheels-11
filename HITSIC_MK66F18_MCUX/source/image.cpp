...
void THRE()
{
    ...
    for (int i = 0; i < 120; i++)
    {
        ...
        for (int j = 0; j < 188; j++)
        {
            if (i < 50 || (j >= head_left && j <= head_right))
            {
                t = threshold + 20;
            }

            if (i > head || j<LEFT_SIDE + 30 || j>RIGHT_SIDE - 30)
            {
                t = threshold - 20;
            }
...
uint8_t find_continue(uint8_t i_start, uint8_t j_start)
{
    ...
    for (j = 1; j <= final_road[i_start - 1].white_num; j++)
    {
            ...
            )
        {
            ...
            width_new = (uint8_t)Max(((int)right - (int)left + 1), 1);
...
GG General_Judge(void)
{
    ...
    for (int i = NEAR_LINE - 5; i > farlength; i--)
    {
...
GG image_main(void)
{
    Start_line = Max(front - length, farlength);
